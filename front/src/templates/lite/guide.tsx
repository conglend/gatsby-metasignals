...
const GuideLite: React.FC<PageProps<GuideLitePageQuery>> = (props) => {
  ...
  const metaData = data?.strapiGuideLite?.MetaData as Strapi__Component_Meta_Metadata

  const global = data?.strapiGlobalLite as Strapi_Global_Lite
  const guide = data?.strapiGuideLite as Strapi_Guide_Lite
...
export default GuideLite

export const query = graphql`
  query GuideLitePage($locale: String) {
    strapiGlobalLite(locale: { eq: $locale }) {
      ...GlobalLiteData
    }
    strapiGuideLite(locale: { eq: $locale }) {
      MetaData {
        MetaTitle
        MetaDescription
      }
