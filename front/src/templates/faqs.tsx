...
const Faqs: React.FC<PageProps<FaqsPageQuery>> = (props) => {
  ...
  const metaData = data?.strapiFaqs?.MetaData as Strapi__Component_Meta_Metadata
  const global = data?.strapiGlobal as Strapi_Global
  const isMobile = useIsMobile()
  const faqs = data?.strapiFaqs?.Faqs
        ...
        <Grid container sx={{ marginBottom: '60px' }}>
          ...
          <Grid item xs={12} sx={{ marginBottom: '36px', marginTop: '36px' }}>
            <Box sx={{ width: { xs: '100%', md: '236px' }, margin: 'auto' }}>
              <Button button={faqs?.FreeTrialButton as Strapi__Component_Links_Button} centered={isMobile} />
            </Box>
          </Grid>
        </Grid>
      </Layout>
